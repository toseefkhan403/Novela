...
class _StoriesPageState extends State<StoriesPage> {
  ...
  List horrorList = List();
  List thrillerList = List();
  List naughtyList = List();
  List steamyList = List();
                ...
                : SingleChildScrollView(
                    ...
                    children: <Widget>[
                      CarouselSlider(
                        ...
                        autoPlay: true,
                        height: 300.0,
                        items: _widgetList(carouselList),
                      ),
                      SizedBox(
                        height: 40.0,
                      ),
                      _epicTitle("Steamy"),
                      _horizontalList(steamyList),
                      SizedBox(
                        height: 30.0,
                      ),
                      _epicTitle("Thriller"),
                      _horizontalList(thrillerList),
                      SizedBox(
                        height: 30.0,
                      ),
                      _epicTitle("Naughty"),
                      _horizontalList(naughtyList),
                      SizedBox(
                        height: 30.0,
                      ),
                      _epicTitle("Horror"),
                      _horizontalList(horrorList),
                      SizedBox(
                        height: 30.0,
                      ),
  ...
  void _initData() async {
    ...
    setState(() {
      List storyList = (snapshot.value as Map<dynamic, dynamic>).values.toList();
      sortLists(storyList);
      _isLoading = false;
    });
  }
  ...
  int getEpisodeNoFromPrefs(String title) =>
      preferences.getInt(title) == null ? 1 : preferences.getInt(title);
    ...
    if (paidUse) {
      ...
      if (snapshot.value == null) {
        ...
        PayUsMoney.showUnlockDialog(
            context, story['title'], 100, '-all episodes', onUnlock: () async {
  ...
  void sortLists(List storyList) {

    storyList.sort((a, b) {
      return b['views'].compareTo(a['views']);
    });

    //horror steamy thriller naughty
    for (int i = 0; i < storyList.length; i++) {

      String storyGenre = storyList[i]['genre'];

      if (storyGenre == "Horror")
        horrorList.add(storyList[i]);

      if (storyGenre == "Steamy")
        steamyList.add(storyList[i]);

      if (storyGenre == "Thriller")
        thrillerList.add(storyList[i]);

      if (storyGenre == "Naughty")
        naughtyList.add(storyList[i]);

      if (i < 5)
        carouselList.add(storyList[i]);
    }
